...
def data_leak(data, target_column="class", corr_limit=0.9, report_file=None):
...
# Function to graph the class imbalance/frequencies 
def graph_class_freq(data, target, report_file, graph_file):
    target_counts = data[target].value_counts()
    with open(report_file, "a") as f:
        f.write(f"\nCounts for each class in the target variable.\n")
        f.write(f"{target_counts}")
    
    #save a graph in the output directory
    plt.figure(figsize=(8, 6))
    target_counts.plot(kind='bar')
    plt.title(f'Class Frequencies for target variable: {target}')
    plt.xlabel('Class')
...
def check_quality(file_path, output_dir):
    ...
    graph_class_freq(data, "class", report_file=output_file, graph_file='../output/class_freq.png')
